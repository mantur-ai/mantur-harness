...
- 客户端沿用[纯文本引用决策](../architecture/2026-07-25-web-input-machine-and-slash-pipeline.zh.md)：菜单 pick 创建一个标签，存在 `title` 时用它作为标签文字，而剪贴板、持久化与模型投影均为字面文本 `/name`；手动键入的 token 发送同一文本。ui-skill 不实现任何裁决钩子。`skills/list`（现在是该领域唯一的 RPC）提供每一个用户可调用的 skill，并携带 `modelInvocable` 与可选的面向用户的 `title`；菜单显示并搜索标题，同时保留 `name` 作为调用值。与宿主命令同名的名称解析为命令——客户端会在该行成为提示词之前完成裁决并将其认领。
