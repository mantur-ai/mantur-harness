...
这个浏览器包添加漫途的首个引导步骤和“漫途账号”设置页。用户可以通过系统浏览器设备 flow 登录，也可以选择“暂时跳过”；两条路径都会把引导交给现有 DeepSeek API Key 步骤。设置页显示经过筛选的账号状态并支持本机退出登录。环境选择是本机 Host 配置，不向浏览器公开。

浏览器只会收到尝试 ID、验证地址、用户码、过期时间、进度和账号邮箱，绝不会收到 ManturHub API Key 或已配置的部署 origin。
