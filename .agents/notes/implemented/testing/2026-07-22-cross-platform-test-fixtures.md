...
Browser RPC failure fixtures return a deterministic failing HTTP status instead of depending on browser-specific timing for an aborted route to reject `fetch()`. The Recipe marketplace loads on mount only when its catalog is idle and preserves a ready catalog's applied query, so a delayed initial request cannot replace a launch failure during slower browser runs. Minimal PowerShell snapshot compositions explicitly disable every unrelated model-tool contributor inherited from the base bundle, including the ordinary PowerShell tool when the persistent variant replaces it, so later base additions cannot silently expand their recorded request headers.
