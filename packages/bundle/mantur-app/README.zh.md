...
漫途侧栏在“项目”之前增加“功能”分组，并固定提供“技能广场”和“配方广场”入口。两个入口分别打开独立空主页面，可以返回当前对话，且不会持久化页面选择。配方页面把配方定义为带有效果样片、提示词模板、可复现算子参数、模型与算子信息和预计复刻成本的优秀验证案例，而不是通用工作流模板。页面说明配方本身免费，算子执行必须先确认实时报价。目录数据、下载和执行能力不属于本 MVP。
