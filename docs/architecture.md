...
The [desktop carrier](../apps/desktop/README.md) remains behind this application path. Its Electron main process starts the built `dsh --profile web` entry on a random loopback port and loads the authenticated URL in a sandboxed native window; it does not mount plugins or implement another Harness runtime.
