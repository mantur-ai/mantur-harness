...
The Recipe page reads ManturHub's public paginated catalog and detail endpoints through the existing Host marketplace Remote. The Host validates all fields, resolves relative media and source paths against the configured Hub response origin, and returns the published `agent_payload` unchanged. The browser owns search, category selection, list and detail state, and page presentation. A Recipe remains a proven creative example rather than an installable package or general workflow template.

“Recreate with Agent” creates a new Session in the current Workspace, opens it, and submits one ordinary user message through the scoped Conversation service. That durable message includes the Recipe title, slug, source URL, and authoritative `agent_payload`; therefore the model sees the same instructions the publisher supplied and the transcript retains enough provenance to audit the handoff. Recipe discovery stays public. The UI does not request login because these endpoints do not require it, and it does not report operator execution as started merely because the Session accepted the message.
...
The shared client gains two generic extension points and one transient page selector, while Mantur copy and page structure remain isolated in product packages. Only one root-page occupant can be composed at a time. Skill catalog reads and safe installation are available in the Mantur profile. Recipe discovery and durable Agent handoff are also available, but operator execution, quote confirmation, and payment remain owned by Agent and ManturHub after the handoff. Forced overwrite, uninstall, and route persistence remain absent.
