...
Browser RPC failure fixtures return a deterministic failing HTTP status instead of depending on browser-specific timing for an aborted route to reject `fetch()`. The Recipe marketplace loads on mount only when its catalog is idle and preserves a ready catalog's applied query, so a delayed initial request cannot replace a launch failure during slower browser runs. The browser snapshot gate pins `Asia/Shanghai` because client time zone is durable session input and the committed Web fixtures record that zone; the focused Cordis fixture also pins its own Playwright context so direct runs preserve the same input. The bounded Web runner completes every build reader before the HMR case rewrites client artifacts. The Windows native CI job runs worker-lifecycle tests and host-exit process-tree tests in separate Vitest invocations, so their fork projects cannot overlap on a hosted runner. The real pwsh PTY cases run only on Windows ConPTY; a POSIX runner with a pwsh binary does not claim to cover that Windows substrate. Minimal PowerShell snapshot compositions explicitly disable every unrelated model-tool contributor inherited from the base bundle, including the ordinary PowerShell tool when the persistent variant replaces it, so later base additions cannot silently expand their recorded request headers.
