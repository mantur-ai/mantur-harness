...
[桌面载体](../apps/desktop/README.zh.md)仍位于这条应用路径之后。其 Electron 主进程会在随机 loopback 端口启动已构建的 `dsh --profile web` 入口，并在 sandbox 原生窗口中加载已认证 URL；它不会挂载插件或实现另一套 Harness 运行时。
