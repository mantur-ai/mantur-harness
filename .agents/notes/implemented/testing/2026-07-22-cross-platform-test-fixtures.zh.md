...
浏览器 RPC 故障 fixture 会返回确定性的失败 HTTP 状态，不再依赖不同浏览器对中止路由何时拒绝 `fetch()` 的时序。配方广场挂载时只会加载处于空闲状态的目录，并保留已就绪目录所应用的查询，因此较慢的浏览器运行中，延迟的初始请求不会覆盖启动失败。最小 PowerShell 快照组合会显式禁用从底座继承的所有无关模型工具贡献者；当持久化 PowerShell 工具替代普通版本时，也会显式禁用普通版本，因此底座后续新增工具不会悄悄扩大已记录的请求头。
